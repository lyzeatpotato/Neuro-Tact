...
public class UnitedUtils {
     ...
     * @return 根据串口更新后的坐标点
     */
    public static Float[] reorderPointList(Float[] pointArr, Float[] portArr) {
        // 首先需要根据传入的串口数据，得到球坐标系的theta/beta值
        Float[][] polarData = new Float[pointArr.length][2];  // 一共144 * 2(每个点对应一个theta跟一个beta)
        try {
            String path = "E:\\Codestation\\Java\\Workspace\\Neuro-Tact\\src\\main\\resources\\txt\\polar.txt";
        ...
        for (int i = 0; i < portArr.length; i++) {  //144次点循环
            ...
            resultPointArr[i * 8 + 0] = new Float(pointArr[i * 8 + 0] + portArr[i] * Math.sin(theta) * Math.cos(beta));
            resultPointArr[i * 8 + 1] = new Float(pointArr[i * 8 + 1] + portArr[i] * Math.sin(theta) * Math.sin(beta));
            resultPointArr[i * 8 + 2] = new Float(pointArr[i * 8 + 2] + portArr[i] * Math.cos(theta));
